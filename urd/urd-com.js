...
module.exports = function (RED) {
    ...
    const pickupErp2DataAsObject = (esp3RawData) => {
        ...
        if (telegramType === 0b0000 || telegramType === 0b0001) { // RPS || 1BS
            ...
            // In case of GP // 0x07: Generic Profiles Complete data (0xB2)
            if (extendedTelegramType === 0x07) {
    ...
    // urd-com node function definition
    function UrdComNode(n) {
        ...
        const propagateReceivedValue = (receivedSensorId, data) => {
        ...
        if (this.serialConfig) {
            ...
            this.port.on('data', (msgout) => {
                ...
                Object.keys(listeners).forEach((nodeId) => {
                    if (nodeId) {
                        const EnObjNode = RED.nodes.getNode(nodeId);
        ...
        UrdComNode.prototype.addLinkData = function (lObj) {
    ...
    RED.nodes.registerType('urd-com', UrdComNode);
};
