    ...
    function EnOceanComNode(n) {
        ...
        if (this.serialConfig) {
            ...
            this.port.on('data', function(msgout) {
                node.send(msgout);
                if (node.linkdataobjConfig) {
                    node.linkdataobjConfig.value = Buffer.from(msgout.payload).toString('hex');
                    node.log(Buffer.from(msgout.payload).toString('hex'));
                }
            });
