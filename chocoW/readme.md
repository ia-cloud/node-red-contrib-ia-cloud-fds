...
チョコ停ウォッチャーから取得した画像や動画データは、ia-cloudオブジェクトの「ファイルデータ」モデルで送出するほか、Node-REDのストレージサービスノードに送り、各種のストレージサービスへ格納することもです。
...
また、以下に「template」ノードと接続して静止画表示をする場合の「template」ノードの「HTMLコード」プロパティの設定例を記す。
```
...
10秒ごとに、エラーコードとアラーム状態を取得する。「アラーム＆イベントを出力する」プロパティがチェックされていれば、エラーコードが変化した時、またはエラーコードがE000(正常)以外のときにia-cloudアラーム＆イベントデータオブジェクトを出力する。*1
...
ノードの3番目の出力は、チョコ停ウォッチャーから取得した静止画・動画ファイルの一時ファイルのファイルパスとストレージサービスへの格納ファイル名を出力する。このメッセージをファイルストレージサービスへの接続Node-REDノードへ繋ぐことによって、これらのファイルをAWSのS3やDropboxなどのストレージサービスへ格納することができる。


## *2: 出力メッセージ3

ノードの3番目の出力は、チョコ停ウォッチャーから取得した静止画を出力する。
...
|ストレージサービスを使う|boolean| ファイルデータをia-cloudオブジェクトではなく、ストレージサービスへ格納するメーっセージ出力を行う|
|ストレージサービス情報|string| ファイルデータを格納するストレージ情報に関する情報。任意に記述ができる。　例："AWS S3: bucket の名前"|
