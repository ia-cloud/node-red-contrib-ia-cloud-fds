...
module.exports = function(RED) {
    ...
    function PLCSim(config) {
        ...
        let PLCSim = {};
        try {
            let plcJson = fs.readFileSync(path.resolve(__dirname, "../PLC-Nodes/PLC-sim.json"), "utf-8");
            PLCSim = JSON.parse(plcJson);
        }
        catch (err) {
            //エラーの場合は、nodeステータスを変更。
            node.status({fill:"red", shape:"ring", text:"runtime.badPLData"});
            node.error(RED._("runtime.badPLCData"));
        }
        ...
        if (interval !== 0 && Object.keys(PLCSim.lenth !== 0)) {
        ...
        function cyclic (gContext, count){
            ...
            // if not PLC-simulator context data exist
            if(!PLCSim) return;
            let mb = PLCSim.modbus;
            let mitsu = PLCSim.mitsubishi;
